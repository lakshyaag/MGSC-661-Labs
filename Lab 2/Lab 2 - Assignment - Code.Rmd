...
plot_dn <- function(data, n) {
  plot <- ggplot(data, aes(x = age, y=avg_rating)) +
    ...
    scale_color_identity(name = "Polynomial", breaks = c("blue"), labels=glue("d-{n}"), guide="legend") +
    theme_classic()
...
plot_dn <- function(data, n) {
  plot <- ggplot(data, aes(x = avg_timeplay, y=avg_rating)) +
    ...
    scale_color_identity(name = "Polynomial", breaks = c("green"), labels=glue("d-{n}"), guide="legend") +
    theme_classic()
...
#### Part B:
```{r}
knots = quantile(avg_timeplay, probs = c(0.25, 0.50, 0.75, 1))
