...
```{r}
model_7 <- lm(score ~ nintendo + strategy + nintendo*strategy)
summary(model_7)
```
...
```{r}
model_8 = lm(score ~ release_year + nintendo + release_year*nintendo)
summary(model_8)
...
plot(release_year, score, pch=2, col=ifelse(nintendo == 1, "green", "blue"), main = glue("score ~ release_year + nintendo + 
                                                                                         release_year*nintendo (R-squared: {format(summary(model_8)$r.squared, digits = 2)})"))
